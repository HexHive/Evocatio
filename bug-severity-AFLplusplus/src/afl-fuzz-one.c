...
7 ——> C-D-Non-byte
*/
int critical_label[] = {2, 3, 4, 5, 6, 7};
...
int count_critical_pos(struct byte_info bMap[], u32 bMap_size) {
  int critical_byte_num = sizeof(critical_label) / sizeof(int);
  int critical_pos_cnt = 0;

  for (u32 i = 0; i < bMap_size; i ++) {
    for (int j = 0; j < critical_byte_num; j++) {
      if (bMap[i].func_lab == critical_label[j]) {
        /* This is a critical positon! */
        critical_pos_cnt++;
        break;
      }
    }
  }
  return critical_pos_cnt;
}
...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  /*****************************************************
   * PARSE CRITICAL BYTES POSITION MAP
   *
   * Before mutation, Let's parse the C-D map first!
   * We will only mutate the critical bytes
   * The position of critical bytes are given by the
   * critical_bytes_pos map
   ******************************************************/

  /*********   Capability Guided Fuzzing Related  *******************/

  /********* End of Capability Guided Fuzzing Related  *****************/
      ...
      switch ((r = rand_below(afl, r_max))) {
...
#endif
          /*********************************************************
           * Capability Oriented Mutation!
           ********************************************************/
           if (start_exploitation) {
             if (!all_union_num) {
               printf("Weird! Should not arrive here!\n");
               FLIP_BIT(out_buf, rand_below(afl, temp_len << 3));
               break;
             }
             /* We will only flip among the union byte */
             // STEP One: random pick an union from union set
             cap_union_t union_picked = rand_union();
             // STEP Two: random pick a byte from that union
             u32 bye_picked = rand_byte_from_union(union_picked);
             // STEP Three: random pick a bit from that byte
             u32 bit_picked = rand_bit_from_byte(bye_picked);

             FLIP_BIT(out_buf, bit_picked);
             break;
           } else {
             FLIP_BIT(out_buf, rand_below(afl, temp_len << 3));
             break;
        ...
        case 4 ... 7: {
           ...
           ********************************************************/
           if (start_exploitation) {
             cap_union_t union_picked = rand_union();
             u32 byte_picked = rand_byte_from_union(union_picked);
             out_buf[byte_picked] =
        ...
        case 10 ... 11: {
          ...
          if (start_exploitation) {
            cap_union_t union_picked = rand_union();
            u32 byte_picked = rand_byte_from_union(union_picked);
            *(u16 *)(out_buf + byte_picked) = SWAP16(
                interesting_16[rand_below(afl, sizeof(interesting_16) >> 1)]);
            break;
          } else {
        ...
        case 14 ... 15: {
          ...
          ********************************************************/
          if (start_exploitation) {
            cap_union_t union_picked = rand_union();
            u32 byte_picked = rand_byte_from_union(union_picked);
            *(u32 *)(out_buf + byte_picked) = SWAP32(
                interesting_32[rand_below(afl, sizeof(interesting_32) >> 2)]);
        ...
        case 16 ... 19: {
          ...
          ********************************************************/
          if (start_exploitation) {
            cap_union_t union_picked = rand_union();
            u32 byte_picked = rand_byte_from_union(union_picked);
            out_buf[byte_picked] -= 1 + rand_below(afl, ARITH_MAX);
        ...
        case 24 ... 25: {
          ...
          ********************************************************/
          ...
          if (start_exploitation) {
            cap_union_t union_picked = rand_union();
            u32 byte_picked = rand_byte_from_union(union_picked);
            *(u16 *)(out_buf + byte_picked) -= 1 + rand_below(afl, ARITH_MAX);
            break;
          } else {
...
#endif
            *(u16 *)(out_buf + byte_picked) =
                SWAP16(SWAP16(*(u16 *)(out_buf + byte_picked)) - num);
        ...
        case 32 ... 33: {
          ...
          ********************************************************/
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
            *(u32 *)(out_buf + byte_picked) -= 1 + rand_below(afl, ARITH_MAX);
            break;
          } else {
...
#endif
            *(u32 *)(out_buf + byte_picked) =
                SWAP32(SWAP32(*(u32 *)(out_buf + byte_picked)) - num);
        ...
        case 40 ... 43: {
          ...
          ********************************************************/
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
            out_buf[byte_picked] ^= 1 + rand_below(afl, 255);
...
abandon_entry:
  ...
  if (!afl->stop_soon && !afl->queue_cur->cal_failed &&
    ...
    if (!afl->queue_cur->was_fuzzed) {
      ...
      update_union_fuzzed(afl->queue_cur->union_hash);
      afl->reinit_table = 1;
      ...
      for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
        ...
        for (i = 0; i < use_stacking; ++i) {
          ...
          switch (select_algorithm(afl, r_max)) {
            ...
            case 0:
              /* Flip a single bit somewhere. Spooky! */
              FLIP_BIT(out_buf, rand_from_map(d_related_pos));
            ...
            case 1:
              if (temp_len < 2) { break; }
              temp_len_puppet = rand_from_map(d_related_pos);
            ...
            case 3:
              if (temp_len < 4) { break; }
              out_buf[rand_from_map(d_related_pos)] ^= 0xFF;
            ...
            case 4:
              if (temp_len < 8) { break; }
              *(u16 *)(out_buf + rand_from_map(d_related_pos)) ^= 0xFFFF;
            ...
            case 5:
              if (temp_len < 8) { break; }
              *(u32 *)(out_buf + rand_from_map(d_related_pos)) ^= 0xFFFFFFFF;
            ...
            case 6:
              out_buf[rand_from_map(d_related_pos)] -=
            ...
            case 7:
              ...
              if (rand_below(afl, 2)) {
                ...
                u32 pos = rand_from_map(d_related_pos);
                *(u16 *)(out_buf + pos) -= 1 + rand_below(afl, ARITH_MAX);
#ifdef INTROSPECTION
                snprintf(afl->m_tmp, sizeof(afl->m_tmp), " ARITH16-%u", pos);
                strcat(afl->mutation, afl->m_tmp);
            ...
            case 8:
              ...
              if (rand_below(afl, 2)) {
                ...
                /* HACK! we didn't check the pos, if pos == len(input),
                 * it will SEGV! */
                u32 pos = rand_from_map(d_related_pos);
#ifdef INTROSPECTION
                snprintf(afl->m_tmp, sizeof(afl->m_tmp), " ARITH32_-%u", pos);
            ...
            case 10:
              ...
              if (rand_below(afl, 2)) {
                ...
                strcat(afl->mutation, afl->m_tmp);
#endif
                *(u16 *)(out_buf + rand_from_map(d_related_pos)) =
            ...
            case 11:
              ...
              if (rand_below(afl, 2)) {
                ...
                strcat(afl->mutation, afl->m_tmp);
#endif
                *(u32 *)(out_buf + rand_from_map(d_related_pos)) =
            ...
            case 12:
              ...
              out_buf[rand_from_map(d_related_pos)] ^= 1 + rand_below(afl, 255);
            ...
            case 13: {
              ...
              /* Delete bytes. We're making this a bit more likely
                 than insertion (the next option) in hopes of keeping
                 files reasonably small. */
              /* Bug severity! we don't delete to keep the input being
               * same length! */
              break;
            ...
            case 14:
                 ...
                 */
                /* Bug severity! we don't insert data to keep the input being
                 * same length! */
            ...
            case 15: {
              ...
              /* TODO: copy from critical, to non-critical position
               *       copy from non-critical, to critical position */
              copy_from = rand_from_map(d_related_pos);
              copy_to = rand_from_map(d_related_pos);
            ...
            case 17: {
              ...
              /* Bug severity! we don't insert to keep the input being
                * same length! */
              break;
