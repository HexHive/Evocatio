...
void update_bitmap_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  /* Capability Hack! */
  /* We don't need to execute this function, because we are not trying to
   * find more new paths, therefore, new paths should not be favorable
   * than the others, we just return this function directly! */
...
/* Find the most element appear in the array */

u32 majorityElement(u32 arr[], u32 arr_len) {
  u32 result = arr[0];
  u32 i = 1, count = 1;
  while (i < arr_len)
  {
...
/* If the latest LATEST_SEED_NUM seed were mutated from one union,
 * we will call this union as the most active union
 *
 * most active union means it is the most valuable union for fuzzing,
 * because it provides you with the most capabilities. */

u32 find_most_active_union(afl_state_t *afl) {
  struct queue_entry *q;
  u32 cur_union_hash = 0;
  u32 arr_to_check[LATEST_SEED_NUM] = {0};
  u32 arr_len = 0;
  u32 winner = 0;

  if (afl->queued_paths <= LATEST_SEED_NUM) {

    /* The number of seeds in the queue is smaller than what we want to check */

    for (u32 i = 0; i < afl->queued_paths; i++) {
      q = afl->queue_buf[i];
      cur_union_hash = q->union_hash;

      arr_to_check[i] = cur_union_hash;

    }

    arr_len = afl->queued_paths;

  } else {

    /* queue is larger than LATEST_SEED_NUM, we only pick the latest seeds */
    for (u32 j = 0; j < LATEST_SEED_NUM; j++) {
      q = afl->queue_buf[j];
      cur_union_hash = q->union_hash;

      arr_to_check[j] = cur_union_hash;

    }

    arr_len = LATEST_SEED_NUM;

  }

  winner = majorityElement(arr_to_check, arr_len);
...
void cull_queue(afl_state_t *afl) {
  /*************************************************************************
   * Bug-severity: Seed Selection
   *
   * We disable the original code of cull_queue, because we don't identify
   * the value of a seed according to its coverage, we only look union
   * instead! We have proposed TWO principles in this part.
   ************************************************************************/
  ...
  /*===============================================================
  * Criteria 1:
  * Seed mutated from new union should be regarded as favorite.
  ===============================================================*/
  u32 fuzzed_union_cnt_ori = fuzzed_union_cnt;
  struct queue_entry *q;
  int is_fuzzed = 0;
  ...
  for (u32 i = 0; i < afl->queued_paths; i++) {
    ...
    if (q->union_len) {
      ...
      /* Check whether we  have fuzzed this union before */
      for (u32 j = 0; j < fuzzed_union_cnt_ori; j++) {

        if (q->union_hash == fuzzed_union[i]) {
          /* We have fuzzed this union before! */
          is_fuzzed = 1;
      ...
      if (!is_fuzzed) {
        /* This union has not been fuzzed, mark it as favorite! */
        q->favored = 1;
        afl->queued_favored++;
        fuzzed_union[fuzzed_union_cnt] = q->union_hash;
        fuzzed_union_cnt += 1;
  ...
  /*===============================================================
  * Criteria 2:
  * Seed mutated from the most active union should be regarded
  * as favorite.
  ===============================================================*/
  /* Re-calculate the most active union first! */
  most_active_union = find_most_active_union(afl);

  for (u32 i = 0; i < afl->queued_paths; i ++) {
    q = afl->queue_buf[i];

    if (q->union_hash == most_active_union) {
      q->favored = 1;
      afl->queued_favored++;
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  /* HACK! Initialize perf_score with 100,
 * Don't care the exec_us */
  u32 perf_score = 100;

  /*************************************************************************
   * Bug-severity: Energy Scheduling
   ************************************************************************/

  /*===============================================================
   * Criteria 1:
   * Seed mutated from longer union will have higher score
   ===============================================================*/
  perf_score = perf_score + perf_score * q->union_len;

  /*===============================================================
   * Criteria 2:
   * Seed mutated from high-heat union will have higher score
   ===============================================================*/
  perf_score += 100 * q->union_heat;

  /* Adjust score based on bitmap size. The working theory is that better
     coverage translates to better targets. Multiplier from 0.25x to 3x. */

  /*************************************************************************
   * Bug-severity: bitmap size should be same in our context
  ...
  /*************************************************************************
   * Bug-severity: handicap should be same in our context
   * So we don't check it!
   *************************************************************************
  if (q->handicap >= 4) {
  ...
  /*************************************************************************
   * Bug-severity: input depth should be same in our context
   * So we don't check it!
   *************************************************************************
  switch (q->depth) {
