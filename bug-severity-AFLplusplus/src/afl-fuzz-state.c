...
/* Read input bytes constraints from file. */

s32 read_constraints_from_file(afl_state_t *afl, struct byte_info byteInfo[]) {
  int pos, func_label, heat;

  cJSON *pJson = NULL;
  cJSON *pTemp = NULL;
  //pJson = prepare_parse_json(afl->cd_res_file);
  if(NULL == pJson)
  {
  ...
  int i;
  for (i = 0; i < cJSON_GetArraySize(pJson)-1; i++) {
    char *buffer = malloc(16);
    snprintf(buffer, 16, "%d", i);
    cJSON *subitem = cJSON_GetObjectItem(pJson, buffer);
    free(buffer);
    ...
    /* Get func_lab */
    pTemp = cJSON_GetObjectItem(subitem,"func_lab");
    func_label = pTemp->valueint;

    /* Get capability_num */
    pTemp = cJSON_GetObjectItem(subitem,"capability_num");
    heat = pTemp->valueint;

    /* Update byteInfo */
    byteInfo[pos].func_lab = func_label;
    byteInfo[pos].heat = (u32)heat;
  }
...
/* Removes this capability state instance and frees it. */
