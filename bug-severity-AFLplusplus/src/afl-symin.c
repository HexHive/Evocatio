...
typedef enum {
    /* 00 */ FIX_VAL,                   /* The value of this byte cannot change  */
    /* 01 */ CHANGEABLE_VAL,            /* The value of this byte is changeable  */
    /* 02 */ PLACE_HOLDER               /* The value of this byte doesn't affect crash or non-crash  */
} FUNCTION_LABEL;
...
struct byte_cons {
    ...
    ATTACKING_LABEL attack_lab[5];

    uint8_t change_range[255];          /* Change range if one byte is changeable_val */
};
...
static s32 write_constraints_to_file(u8 *path, struct byte_cons *consRes, u32 len) {
    ...
    for (int i = 0; i < arr_len; i++) {
        ...
        /* Prepare attack_lab */
        cJSON *array_attack = NULL;
        cJSON_AddItemToObject(elem,"attack_lab",array_attack=cJSON_CreateArray());
        for (uint8_t j = 0; j < sizeof(consRes->attack_lab)/sizeof(ATTACKING_LABEL); j++) {
            cJSON_AddItemToArray(array_attack,cJSON_CreateNumber(consRes[i].attack_lab[j]));
        }

        /* Prepare change range */
        cJSON *array_val = NULL;
        cJSON_AddItemToObject(elem,"val_range",array_val=cJSON_CreateArray());
        for (int n = 0; n < 255; n++) {
            int v = consRes[i].change_range[n];
            if (v == 1)
                cJSON_AddItemToArray(array_val, cJSON_CreateNumber(n));
        }
    }
...
static u8 tmin_run_target(afl_forkserver_t *fsrv, u8 *mem, u32 len,
  ...
  if (orig_cksum == cksum && ori_hit_count == cur_hit_count) { return 1; }
...
next_del_blksize:
  ...
  if (cur_pass > 1 && !changed_any) { goto bruteforce_all; }
  ...
  if (!brute_end_pos) {
    ...
    for (i = brute_start_pos; i < brute_end_pos; i++) {
        ...
        for (int j = 0; j < 256; j++) {
            ...
            if (orig == j) {
                constraintsRes[i].change_range[j] = 1;  /* update change range */
                pass_val_cnt++;
            ...
            res = tmin_run_target(fsrv, tmp_buf, in_len, 0);
            if (res) {
                constraintsRes[i].func_lab = CHANGEABLE_VAL;
                constraintsRes[i].change_range[j] = 1;  /* update change range */
                pass_val_cnt++;
        ...
        /* We have verified 256 values, then we can give the final decision about whether it is a FIX_VAL */
        if (constraintsRes[i].func_lab != CHANGEABLE_VAL) {
            constraintsRes[i].func_lab = FIX_VAL;
            constraintsRes[i].change_range[orig] = 1;  /* change range should be only one */
        } else {
          /* PLACE_HOLDER is a special case for CHANGEABLE_VAL */
          if (pass_val_cnt == 256) {
            /* This should be a PLACE_HOLDER! */
            constraintsRes[i].func_lab = PLACE_HOLDER;
          }
        }
    }
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:c:s:d:f:m:t:BxeOQUWHh")) > 0) {
